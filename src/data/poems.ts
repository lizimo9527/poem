import type { Poem, Author } from '@/types/poem'
import { PoemService } from '@/services/poemService'

// 本地备用数据（当数据库不可用时使用）
export const fallbackPoems: Poem[] = [
  {
    id: 1,
    title: '静夜思',
    author: '李白',
    dynasty: '唐',
    content: '床前明月光，疑是地上霜。举头望明月，低头思故乡。',
    tags: ['思乡', '月亮'],
    translation:
      '明亮的月光洒在窗户纸上，好像地上泛起了一层霜。我禁不住抬起头来，看那天窗外空中的一轮明月，不由得低头沉思，想起远方的家乡。',
    explanation:
      '此诗描写了秋日夜晚，诗人于屋内抬头望月的所感。诗中运用比喻、衬托等手法，表达客居思乡之情。',
  },
  {
    id: 2,
    title: '春晓',
    author: '孟浩然',
    dynasty: '唐',
    content: '春眠不觉晓，处处闻啼鸟。夜来风雨声，花落知多少。',
    tags: ['春天', '田园'],
    translation:
      '春日里贪睡不知不觉天已破晓，搅乱我酣眠的是那啁啾的小鸟。昨天夜里风声雨声一直不断，那娇美的春花不知被吹落了多少？',
    explanation:
      '诗人抓住春天的早晨刚刚醒来时的一瞬间展开描写和联想，生动地表达了诗人对春天的热爱和怜惜之情。',
  },
  {
    id: 3,
    title: '登鹳雀楼',
    author: '王之涣',
    dynasty: '唐',
    content: '白日依山尽，黄河入海流。欲穷千里目，更上一层楼。',
    tags: ['登高', '哲理'],
    translation:
      '夕阳依傍着西山慢慢地沉没，滔滔黄河朝着东海汹涌奔流。若想把千里的风光景物看够，那就要登上更高的一层城楼。',
    explanation:
      '此诗前两句写所见，后两句写所感，把哲理与景物、情势溶化得天衣无缝，成为鹳雀楼上一首不朽的绝唱。',
  },
  {
    id: 4,
    title: '望庐山瀑布',
    author: '李白',
    dynasty: '唐',
    content: '日照香炉生紫烟，遥看瀑布挂前川。飞流直下三千尺，疑是银河落九天。',
    tags: ['山水', '瀑布', '壮观'],
    translation:
      '香炉峰在阳光的照射下生起紫色烟霞，远远望见瀑布似白色绢绸悬挂在山前。高崖上飞腾直落的瀑布好像有几千尺，让人恍惚以为银河从天上泻落到人间。',
    explanation: '这首诗形象地描绘了庐山瀑布雄奇壮丽的景色，反映了诗人对祖国大好河山的热爱。',
  },
  {
    id: 5,
    title: '春夜喜雨',
    author: '杜甫',
    dynasty: '唐',
    content: '好雨知时节，当春乃发生。随风潜入夜，润物细无声。',
    tags: ['春天', '雨', '喜悦'],
    translation:
      '好雨似乎会挑选时辰，降临在万物萌生之春。伴随和风，悄悄进入夜幕。细细密密，滋润大地万物。',
    explanation: '这首诗描写春夜降雨、润泽万物的美景，抒发了诗人对春雨的喜悦之情。',
  },
  {
    id: 6,
    title: '水调歌头',
    author: '苏轼',
    dynasty: '宋',
    content: '明月几时有？把酒问青天。不知天上宫阙，今夕是何年。',
    tags: ['中秋', '月亮', '思念'],
    translation:
      '明月从什么时候才开始出现的？我端起酒杯遥问苍天。不知道在天上的宫殿，今天晚上是何年何月。',
    explanation: '此词是中秋望月怀人之作，表达了对胞弟苏辙的无限怀念。',
  },
  {
    id: 7,
    title: '声声慢',
    author: '李清照',
    dynasty: '宋',
    content: '寻寻觅觅，冷冷清清，凄凄惨惨戚戚。乍暖还寒时候，最难将息。',
    tags: ['秋天', '忧愁', '孤独'],
    translation: '苦苦地寻寻觅觅，却只见冷冷清清，怎不让人凄惨悲戚。乍暖还寒的时节，最难保养休息。',
    explanation:
      '这首词通过描写残秋所见、所闻、所感，抒发自己因国破家亡、天涯沦落而产生的孤寂落寞、悲凉愁苦的心绪。',
  },
  {
    id: 8,
    title: '将进酒',
    author: '李白',
    dynasty: '唐',
    content: '君不见黄河之水天上来，奔流到海不复回。君不见高堂明镜悲白发，朝如青丝暮成雪。',
    tags: ['饮酒', '豪放', '人生'],
    translation:
      '你难道看不见那黄河之水从天上奔腾而来，波涛翻滚直奔东海从来不会再往回流。你难道看不见那年迈的父母，对着明镜悲叹自己的白发，早晨还是满头的黑发，怎么才到傍晚就变成了雪白一片。',
    explanation:
      '此诗为李白长安放还以后所作，思想内容非常深沉，艺术表现非常成熟，在同题作品中影响最大。',
  },
  {
    id: 9,
    title: '相思',
    author: '王维',
    dynasty: '唐',
    content: '红豆生南国，春来发几枝。愿君多采撷，此物最相思。',
    tags: ['相思', '友情', '爱情'],
    translation:
      '红豆生长在阳光明媚的南方，每逢春天不知长多少新枝。希望思念的人儿多多采摘，因为它最能寄托相思之情。',
    explanation:
      '这是借咏物而寄相思的诗，是眷怀友人之作。全诗情调健美高雅，怀思饱满奔放，语言朴素无华，韵律和谐柔美。',
  },
  {
    id: 10,
    title: '清明',
    author: '杜牧',
    dynasty: '唐',
    content: '清明时节雨纷纷，路上行人欲断魂。借问酒家何处有？牧童遥指杏花村。',
    tags: ['清明', '节日', '忧愁'],
    translation:
      '江南清明时节细雨纷纷飘洒，路上羁旅行人个个落魄断魂。借问当地之人何处买酒浇愁？牧童笑而不答遥指杏花山村。',
    explanation:
      '这首小诗，一个难字也没有，一个典故也不用，整篇是十分通俗的语言，写得自如之极，毫无经营造作之痕。',
  },
  {
    id: 11,
    title: '题西林壁',
    author: '苏轼',
    dynasty: '宋',
    content: '横看成岭侧成峰，远近高低各不同。不识庐山真面目，只缘身在此山中。',
    tags: ['哲理', '庐山', '观察'],
    translation:
      '从正面、侧面看庐山山岭连绵起伏、山峰耸立，从远处、近处、高处、低处看都呈现不同的样子。之所以辨不清庐山真正的面目，是因为我身处在庐山之中。',
    explanation: '这是一首诗中有画的写景诗，又是一首哲理诗，哲理蕴含在对庐山景色的描绘之中。',
  },
  {
    id: 12,
    title: '山行',
    author: '杜牧',
    dynasty: '唐',
    content: '远上寒山石径斜，白云生处有人家。停车坐爱枫林晚，霜叶红于二月花。',
    tags: ['秋天', '山林', '美景'],
    translation:
      '沿着弯弯曲曲的小路上山，在那生出白云的地方居然还有几户人家。停下马车是因为喜爱深秋枫林的晚景，枫叶秋霜染过，艳比二月春花。',
    explanation:
      '这首诗描绘的是秋之色，展现出一幅动人的山林秋色图。诗里写了山路、人家、白云、红叶，构成一幅和谐统一的画面。',
  },
  {
    id: 13,
    title: '枫桥夜泊',
    author: '张继',
    dynasty: '唐',
    content: '月落乌啼霜满天，江枫渔火对愁眠。姑苏城外寒山寺，夜半钟声到客船。',
    tags: ['秋天', '夜晚', '忧愁'],
    translation:
      '月亮已落下乌鸦啼叫寒气满天，江边枫树与船上渔火，难抵我独自傍愁而眠。姑苏城外那寂寞清静寒山古寺，半夜里敲响的钟声传到了我乘坐的客船。',
    explanation:
      '此诗精确而细腻地描述了一个客船夜泊者对江南深秋夜景的观察和感受，勾画了月落乌啼、霜天寒夜、江枫渔火、孤舟客子等景象。',
  },
  {
    id: 14,
    title: '游子吟',
    author: '孟郊',
    dynasty: '唐',
    content: '慈母手中线，游子身上衣。临行密密缝，意恐迟迟归。谁言寸草心，报得三春晖。',
    tags: ['母爱', '亲情', '感恩'],
    translation:
      '慈母用手中的针线，为远行的儿子赶制身上的衣衫。临行前一针针密密地缝缀，怕的是儿子回来得晚衣服破损。有谁敢说，子女像小草那样微弱的孝心，能够报答得了像春晖普泽的慈母恩情呢？',
    explanation:
      '这是一首母爱的颂歌。全诗共六句三十字，采用白描的手法，通过回忆一个看似平常的临行前缝衣的场景，凸显并歌颂了母爱的伟大与无私。',
  },
  {
    id: 15,
    title: '黄鹤楼送孟浩然之广陵',
    author: '李白',
    dynasty: '唐',
    content: '故人西辞黄鹤楼，烟花三月下扬州。孤帆远影碧空尽，唯见长江天际流。',
    tags: ['送别', '友情', '长江'],
    translation:
      '友人在黄鹤楼与我辞别，在柳絮如烟、繁花似锦的阳春三月去扬州远游。孤船帆影渐渐消失在碧空尽头，只看见滚滚长江向天际奔流。',
    explanation:
      '这首送别诗有它特殊的感情色调。它不同于王勃《送杜少府之任蜀川》那种少年刚肠的离别，也不同于王维《渭城曲》那种深情体贴的离别。',
  },
  {
    id: 16,
    title: '江雪',
    author: '柳宗元',
    dynasty: '唐',
    content: '千山鸟飞绝，万径人踪灭。孤舟蓑笠翁，独钓寒江雪。',
    tags: ['冬天', '孤独', '雪景'],
    translation:
      '所有的山上，飞鸟的身影已经绝迹，所有道路都不见人的踪迹。江面孤舟上，一位披戴着蓑笠的老翁，独自在大雪覆盖的寒冷江面上垂钓。',
    explanation:
      '诗人只用了二十个字，就描绘了一幅幽静寒冷的画面：在下着大雪的江面上，一叶小舟，一个老渔翁，独自在寒冷的江心垂钓。',
  },
  {
    id: 17,
    title: '凉州词',
    author: '王翰',
    dynasty: '唐',
    content: '葡萄美酒夜光杯，欲饮琵琶马上催。醉卧沙场君莫笑，古来征战几人回？',
    tags: ['边塞', '战争', '豪迈'],
    translation:
      '酒筵上甘醇的葡萄美酒盛满在夜光杯之中，正要畅饮时，马上琵琶也声声响起，仿佛催人出征。如果醉卧在沙场上，也请你不要笑话，古来出外打仗的能有几人返回家乡？',
    explanation:
      '这首诗是咏边塞情景之名篇。全诗写艰苦荒凉的边塞的一次盛宴，描摹了征人们开怀痛饮、尽情酣醉的场面。',
  },
  {
    id: 18,
    title: '无题',
    author: '李商隐',
    dynasty: '唐',
    content: '相见时难别亦难，东风无力百花残。春蚕到死丝方尽，蜡炬成灰泪始干。',
    tags: ['爱情', '离别', '思念'],
    translation:
      '相见很难，离别更难，何况在这东风无力、百花凋谢的暮春时节。春蚕结茧到死时丝才吐完，蜡烛要烧成灰烬时像泪一样的蜡油才能滴干。',
    explanation:
      '这首诗，以女性的口吻抒写爱情心理，在悲伤、痛苦之中，寓有灼热的渴望和坚忍的执着精神，感情境界深微绵邈，极为丰富。',
  },
  {
    id: 19,
    title: '虞美人',
    author: '李煜',
    dynasty: '五代',
    content: '春花秋月何时了？往事知多少。小楼昨夜又东风，故国不堪回首月明中。',
    tags: ['亡国', '忧愁', '思念'],
    translation:
      '这年的时光什么时候才能了结，往事知道有多少！昨夜小楼上又吹来了春风，在这皓月当空的夜晚，怎承受得了回忆故国的伤痛。',
    explanation:
      '此词为作者绝笔，是一曲生命的哀歌，作者通过对自然永恒与人生无常的尖锐矛盾的对比，抒发了亡国后顿感生命落空的悲哀。',
  },
  {
    id: 20,
    title: '出塞',
    author: '王昌龄',
    dynasty: '唐',
    content: '秦时明月汉时关，万里长征人未还。但使龙城飞将在，不教胡马度阴山。',
    tags: ['边塞', '战争', '爱国'],
    translation:
      '依旧是秦汉时期的明月和边关，守边御敌鏖战万里征人未回还。倘若龙城的飞将李广如今还在，绝不许匈奴南下牧马度过阴山。',
    explanation:
      '这首诗虽然只有短短四行，但是通过对边疆景物和征人心理的描绘，表现的内容是复杂的。既有对久戍士卒的浓厚同情和结束这种边防不顾局面的愿望。',
  },
]

export const fallbackAuthors: Author[] = [
  {
    id: 1,
    name: '李白',
    dynasty: '唐',
    intro: '李白（701年－762年），字太白，号青莲居士，唐代伟大的浪漫主义诗人，被后人誉为"诗仙"。',
    poemsCount: 987,
  },
  {
    id: 2,
    name: '孟浩然',
    dynasty: '唐',
    intro: '孟浩然（689年－740年），唐代著名的山水田园派诗人，与王维并称"王孟"。',
    poemsCount: 321,
  },
  {
    id: 3,
    name: '王之涣',
    dynasty: '唐',
    intro: '王之涣（688年－742年），盛唐时期的著名诗人，以描写边塞风光著称。',
    poemsCount: 6,
  },
  {
    id: 4,
    name: '杜甫',
    dynasty: '唐',
    intro: '杜甫（712年－770年），字子美，自号少陵野老，唐代伟大的现实主义诗人，被后人誉为"诗圣"。',
    poemsCount: 1150,
  },
  {
    id: 5,
    name: '苏轼',
    dynasty: '宋',
    intro: '苏轼（1037年－1101年），字子瞻，号东坡居士，北宋著名文学家、书画家。',
    poemsCount: 2700,
  },
  {
    id: 6,
    name: '李清照',
    dynasty: '宋',
    intro: '李清照（1084年－1155年），号易安居士，宋代著名女词人，婉约派代表。',
    poemsCount: 78,
  },
  {
    id: 7,
    name: '王维',
    dynasty: '唐',
    intro: '王维（701年－761年），字摩诘，号摩诘居士，唐代著名诗人、画家。',
    poemsCount: 400,
  },
  {
    id: 8,
    name: '杜牧',
    dynasty: '唐',
    intro: '杜牧（803年－852年），字牧之，号樊川居士，晚唐著名诗人。',
    poemsCount: 500,
  },
  {
    id: 9,
    name: '柳宗元',
    dynasty: '唐',
    intro: '柳宗元（773年－819年），字子厚，唐代文学家、哲学家，唐宋八大家之一。',
    poemsCount: 140,
  },
  {
    id: 10,
    name: '王昌龄',
    dynasty: '唐',
    intro: '王昌龄（698年－757年），字少伯，盛唐著名边塞诗人。',
    poemsCount: 180,
  },
  {
    id: 11,
    name: '张继',
    dynasty: '唐',
    intro: '张继（约715年－779年），字懿孙，唐代诗人，以《枫桥夜泊》闻名。',
    poemsCount: 50,
  },
  {
    id: 12,
    name: '孟郊',
    dynasty: '唐',
    intro: '孟郊（751年－814年），字东野，唐代著名诗人，与贾岛并称"郊寒岛瘦"。',
    poemsCount: 500,
  },
  {
    id: 13,
    name: '王翰',
    dynasty: '唐',
    intro: '王翰（687年－726年），字子羽，唐代边塞诗人。',
    poemsCount: 14,
  },
  {
    id: 14,
    name: '李商隐',
    dynasty: '唐',
    intro: '李商隐（约813年－约858年），字义山，号玉谿生，晚唐著名诗人。',
    poemsCount: 600,
  },
  {
    id: 15,
    name: '李煜',
    dynasty: '五代',
    intro: '李煜（937年－978年），南唐后主，五代十国时南唐国君，著名词人。',
    poemsCount: 70,
  },
]

// 向后兼容的导出（保持原有接口不变）
export const poems: Poem[] = fallbackPoems
export const authors: Author[] = fallbackAuthors

// 获取诗词数据的函数（优先使用数据库，失败时使用备用数据）
export const getPoems = async (): Promise<Poem[]> => {
  try {
    return await PoemService.getAllPoems()
  } catch (error) {
    console.warn('数据库连接失败，使用本地备用数据:', error)
    return fallbackPoems
  }
}

// 获取作者数据的函数
export const getAuthors = async (): Promise<Author[]> => {
  try {
    return await PoemService.getAllAuthors()
  } catch (error) {
    console.warn('数据库连接失败，使用本地备用作者数据:', error)
    return fallbackAuthors
  }
}

// 根据ID获取诗词
export const getPoemById = async (id: number): Promise<Poem | null> => {
  try {
    const poem = await PoemService.getPoemById(id)
    return poem || fallbackPoems.find((p) => p.id === id) || null
  } catch (error) {
    console.warn('数据库连接失败，使用本地备用数据查找诗词:', error)
    return fallbackPoems.find((p) => p.id === id) || null
  }
}

// 搜索诗词
export const searchPoems = async (query: string): Promise<Poem[]> => {
  try {
    return await PoemService.searchPoems(query)
  } catch (error) {
    console.warn('数据库搜索失败，使用本地备用数据搜索:', error)
    return fallbackPoems.filter(
      (poem) =>
        poem.title.includes(query) || poem.author.includes(query) || poem.content.includes(query)
    )
  }
}

// 获取随机诗词
export const getRandomPoem = async (): Promise<Poem | null> => {
  try {
    const poem = await PoemService.getRandomPoem()
    return poem || fallbackPoems[Math.floor(Math.random() * fallbackPoems.length)] || null
  } catch (error) {
    console.warn('数据库连接失败，使用本地备用随机诗词:', error)
    return fallbackPoems[Math.floor(Math.random() * fallbackPoems.length)] || null
  }
}
